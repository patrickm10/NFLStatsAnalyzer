...
def get_weekly_rb_stats(player_name):
...
def get_career_rushing_stats(player_name):
    ...
    try:
        # Extract the first table (rushing stats) and save to CSV
        df_rushing = pd.read_html(str(tables[0]), flavor='lxml')[0]
...
def get_career_receiving_stats(player_name):
    ...
    try:
        # Extract the second table (receiving stats) and save to CSV
        df_receiving = pd.read_html(str(tables[1]), flavor='lxml')[0]
...
for index, row in df.iterrows():
    ...
    get_weekly_rb_stats(player_name)
